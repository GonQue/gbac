...
class Agent(nn.Module):
    ...
    def get_action(self, x, action=None):
        ...
        if action is None:
            action = probs.sample()
        return action, probs.log_prob(action).sum(1), probs.entropy().sum(1), action_std
...
for update in range(1, num_updates + 1):
    ...
    for step in range(0, args.num_steps):
        ...
        with torch.no_grad():
            ...
            action, logproba, _, action_std = agent.get_action(obs[step])
    ...
    for i_epoch_pi in range(args.update_epochs):
        ...
        for start in range(0, args.batch_size, args.minibatch_size):
            ...
            _, newlogproba, entropy, _ = agent.get_action(b_obs[minibatch_ind], b_actions[minibatch_ind])
...
while total_episodes < args.eval_episodes:
    ...
    next_obs, _, _, info = test_envs.step(action)
