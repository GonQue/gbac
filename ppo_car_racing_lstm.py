...
experiment_name = f"{args.gym_id}__{args.filename}{full_image_str}__{args.seed}__{int(time.time())}"
if args.track:
    import wandb
    wandb.init(
...
class Agent(nn.Module):
    def __init__(self, num_channels, frames=1):
        super(Agent, self).__init__()
        
        self.network = nn.Sequential(
            layer_init(nn.Conv2d(num_channels * frames, 32, 8, stride=4)),
            nn.ReLU(),
    ...
    def get_action(self, x, lstm_state, done, action=None):
        ...
        if action is None:
            action = probs.sample()
        return action, probs.log_prob(action).sum(1), probs.entropy().sum(1), lstm_state, action_std

    def get_value(self, x, lstm_state, done):
        hidden, _ = self.get_states(x, lstm_state, done)
        return self.critic(hidden)

agent = Agent(envs.observation_space.shape[0]).to(device)
...
for update in range(1, num_updates + 1):
    ...
    for step in range(0, args.num_steps):
        ...
        with torch.no_grad():
            values[step] = agent.get_value(obs[step], next_lstm_state, next_done).flatten()
            action, logproba, _, next_lstm_state, action_std = agent.get_action(obs[step], next_lstm_state, next_done)
    ...
    with torch.no_grad():
        last_value = agent.get_value(next_obs.to(device), next_lstm_state, next_done).reshape(1, -1)
        if args.gae:
    ...
    target_agent = Agent(envs.observation_space.shape[0]).to(device)
...
agent = Agent(test_envs.observation_space.shape[0]).to(device)
...
while total_episodes < args.eval_episodes:
    ...
    with torch.no_grad():
        action, _, _, next_lstm_state, _ = agent.get_action(next_obs, next_lstm_state, next_done)
