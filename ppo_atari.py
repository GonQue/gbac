...
def make_env(gym_id, seed, idx, capture_video, resize, run_name, test=False):
    def thunk():
        ...
        if gym_id[-2:] == "v4":      
            ...
            env = MaxAndSkipEnv(env, skip=4) # Technique introduced to save computational time (does the frameskip) (From DQN Mnih paper)
        ...
        env = FrameStack(env, 4) # Can help the agent identify the velocity of moving objects
        env.seed(seed)
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.network = nn.Sequential(
            layer_init(nn.Conv2d(4, 32, kernel_size=(k1_h, k1_w), stride=(s1_h, s1_w))),
            nn.ReLU(),
...
if __name__ == "__main__":
    ...
    while total_episodes < args.eval_episodes:
        ...
        next_obs, _, _, info = test_envs.step(action.cpu().numpy())
