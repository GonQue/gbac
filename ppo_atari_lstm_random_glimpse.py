...
class Agent(nn.Module):
    def __init__(self, envs, num_glimpses, glimpse_h, glimpse_w, num_patches, scaling_factor):
    ...
    def get_states(self, x, lstm_state, prev_loc, done, save_glimpses=False):
        phi, bboxes = self.retina.foveate(x / 255.0, prev_loc, save_glimpses)
    ...
    def get_action_and_value(self, x, lstm_state, prev_loc, done, action=None, loc=None, save_glimpses=False):
        ...
        return action, probs.log_prob(action), probs.entropy(), loc, self.critic(hidden), lstm_state, bboxes
...
if __name__ == "__main__":
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, logprob, _, loc, value, next_lstm_state, bboxes = agent.get_action_and_value(next_obs, next_lstm_state, prev_locs[step], next_done)
        ...
        with torch.no_grad():
            next_value = agent.get_value(next_obs, next_lstm_state, locs[step], next_done).reshape(1, -1)
            if args.gae:
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.num_envs, envsperbatch):
                ...
                _, newlogprob, entropy, _, newvalue, _, _ = agent.get_action_and_value(
                    b_obs[mb_inds], 
    ...
    while total_episodes < args.eval_episodes:
        ...
        with torch.no_grad():
            action, _, _, loc, _, next_lstm_state, bboxes = agent.get_action_and_value(next_obs, next_lstm_state, loc, next_done, save_glimpses=save_glimpses)
